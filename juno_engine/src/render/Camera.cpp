...
{
	 this->position.x = pos.x * sinf(juno::toRadians(dRot.y));
	 this->position.z = pos.z * cosf(juno::toRadians(dRot.y));
	 this->pitch = dRot.y;
	 this->yaw = dRot.z;
	 this->roll = dRot.x;
}
...
void Camera::update(glm::vec3& dPos, glm::vec3& dRot, float deltaZoom, MouseInputData &mouse_data)
{
	...
	this->pitch += dRot.y;
	this->yaw += dRot.z;
	this->position.x += dPos.x; // * sinf(juno::toRadians(yaw)); // * cosf(juno::toRadians(yaw)) + velocity.z * sinf(juno::toRadians(yaw));
...
{
	float rad_theta = juno::toRadians(pitch);// juno::toRadians(pitch);
	...
	float cam_x_offset = dist_x * sin(juno::toRadians(cam_angle));
	float cam_z_offset = dist_x * cos(juno::toRadians(cam_angle));
